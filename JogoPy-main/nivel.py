...
class Nivel:
    ...
    def setup(self, tmx_map):
        ...
        # objetos
        if tmx_map.get_layer_by_name("Player"):
    ...
    def map_change(self, num_nivel):
            if self.player.hitbox_rect.colliderect(self.level_finish_rect) and self.player2.hitbox_rect.colliderect(self.level_finish_rect):
                  #  num_nivel = 1 + num_nivel

                  #  print(num_nivel)
                    self.switch_stage(num_nivel)
    def press_botao(self):
        if self.player.hitbox_rect.colliderect(self.botao) or self.player2.hitbox_rect.colliderect(self.botao):
